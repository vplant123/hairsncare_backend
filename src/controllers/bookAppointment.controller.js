...
const bookAppointmentContactUs = asyncHandler(async (req, res) => {
    try {
        ...
        let record = await zohoService.createRecord({"module":"Leads",reqData : reqData})
        ...
        let sendEmail1 = await sendEmail("hairsncares@gmail.com", 'New Appointment for Hairsncares - Contact Us',`New Appointment Request\n\n
            Name : ${name || ""},\n Phone : ${phone || ""},\n Email : ${email || ""},\n Message: ${msg || ""},\n Preferred method : ${method} `);
        console.log("kfoker",sendEmail1)

        if(email){
            let sendEmail2 = await sendEmail(email, 'Thank you for contacting Hairsncares.com',`Our expert will get in touch shortly`);
